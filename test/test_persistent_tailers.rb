...
describe 'Mongoriver::PersistentTailer' do
  ...
  describe 'reading and storing state' do
    ...
    it 'should update gracefully' do
      ...
      @tailer.expects(:most_recent_operation).with(Time.at(77))

      assert_equal(Time.at(77), @tailer.read_state['time'])
    end
  end

  it 'helper methods for timestamps/placeholders' do
    @state_collection.expects(:find_one).returns({
      'state' => @state,
      'v' => 1
    }).at_least_once
    ...
    assert_equal(@state['placeholder'], @tailer.read_placeholder)
  end

  it 'should tail from placeholder' do
    @tailer.expects(:read_placeholder).returns('foo')
